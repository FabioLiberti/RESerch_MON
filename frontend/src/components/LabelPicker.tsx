...
// -----------------------------------------------------------------------------
// MultiLabelEditor — same UX as the paper detail page (`/papers/[id]`):
// • Renders assigned labels as pill chips (tinted background using the label colour, "×" to remove)
// • A "+ Add Label" dashed pill opens an inline dropdown with search + create-on-the-fly
// • Filters out already-assigned labels from the dropdown
// Use this anywhere you want to manage a *list* of labels (vs LabelPicker which is single-select).
// -----------------------------------------------------------------------------

interface MultiLabelEditorProps {
  value: number[];
  onChange: (ids: number[]) => void;
  // Optional label shown to the left of the chips ("Labels", "with labels:", etc.)
  // Pass null/empty to omit.
  label?: string | null;
...
export function MultiLabelEditor({
  ...
  const assigned = (allLabels || []).filter((l) => assignedIds.has(l.id));
  ...
  // An exact match anywhere (assigned or not) suppresses "Create new" to avoid duplicates.
  ...
  const createAndAssign = async () => {
    ...
    if (!name) return;
    try {
      const newLabel = await api.createLabel({ name, color: newColor });
      mutate("/api/v1/labels");
          ...
          <>
            ...
            <div className="absolute left-0 top-8 z-50 w-72 rounded-xl bg-[var(--card)] border border-[var(--border)] shadow-xl overflow-hidden">
              <div className="p-2 border-b border-[var(--border)]">
                ...
                {query && (
                  <div className="mt-1 text-[9px] text-[var(--muted-foreground)]">
                    {filtered.length} of {available.length} match
                    {hasExactMatch && <span className="ml-2 text-amber-400">· exact match exists</span>}
                  </div>
                )}
              </div>
              ...
              <div className="max-h-64 overflow-y-auto py-1">
                {filtered.length === 0 && !canCreate && (
                  <div className="px-3 py-3 text-[11px] text-[var(--muted-foreground)] text-center">
                    No labels{query ? " match" : " available"}.
                  </div>
                )}
                {filtered.map((l, i) => (
                  <button
                    ...
                    onClick={() => assign(l.id)}
                    onMouseEnter={() => setHighlight(i)}
                    className={`w-full flex items-center gap-2 px-3 py-1.5 text-left transition-colors ${highlight === i ? "bg-[var(--secondary)]" : "hover:bg-[var(--secondary)]"}`}
                  >
                    <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: l.color }} />
