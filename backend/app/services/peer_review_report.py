...
def build_txt(pr: PeerReview) -> str:
    ...
    # Footer (configurable template + signature/affiliation/extra/timestamp)
    lines.append("-" * 72)
    lines.append(render_pdf_footer(latex=False))
    lines.append("")
...
def build_md(pr: PeerReview) -> str:
    ...
    out.append("---")
    out.append(f"*{render_pdf_footer(latex=False)}*")
    out.append("")
...
def build_tex(pr: PeerReview) -> str:
    ...
    footer_line = render_pdf_footer(latex=True)
